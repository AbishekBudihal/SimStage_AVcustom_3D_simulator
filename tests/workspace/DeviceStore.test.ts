...
const input: PlacedDevice = {
  ...
  ports: [{ id: "power", label: "Power", signal: "AC", direction: "input" }],
  metadata: {
    label: "Rack",
    powerWatts: null,
    heatBtuPerHour: null,
...
describe("workspace state", () => {
  ...
  it("snaps in half-metre increments and validates grid size", () => {
    expect(
      snapToSurface({ x: 0.74, y: 1, z: -0.76 }, "floor", {
        width: 6,
        depth: 4,
        height: 3,
      }),
...
describe("Zustand actions", () => {
  it("merges transforms and metadata without mutating earlier snapshots or other devices", () => {
    ...
    const patch = {
      ...
      metadata: { rackUnits: 4, powerWatts: 120 },
    };
    ...
    expect(after.devices[input.id]?.metadata).toEqual({
      ...input.metadata,
      rackUnits: 4,
      powerWatts: 120,
    });
  ...
  it("validates rotation and engineering metadata before publishing updates", () => {
    ...
    const before = store.api.getState();
    expect(() =>
      store.update(input.id, { rotation: { z: Infinity } }),
    ).toThrow();
    expect(() =>
      store.update(input.id, { metadata: { rackUnits: -1 } }),
    ).toThrow();
    expect(() =>
      store.update(input.id, { metadata: { heatBtuPerHour: NaN } }),
    ).toThrow();
