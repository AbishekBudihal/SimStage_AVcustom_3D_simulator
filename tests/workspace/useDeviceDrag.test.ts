...
import { DeviceStore, type MountSurface, type XYZ } from '../../src/workspace/DeviceStore';
import { useDeviceDrag } from '../../src/workspace/useDeviceDrag';
import type { CanvasManager } from '../../src/workspace/CanvasManager';

let cleanup = () => {};
afterEach(() => { cleanup(); vi.unstubAllGlobals(); });
function setup(surface: MountSurface = 'floor', origin: XYZ = { x: 0, y: 0, z: 0 }) {
  const listeners = new Map<string, (event: any) => void>();
  const windowListeners = new Map<string, (event: any) => void>();
  const captures = new Set<number>();
  const element = {
    style: { cursor: 'grab' },
    getBoundingClientRect: () => ({ left: 30, top: 20, width: 800, height: 600 }),
  ...
  camera.position.set(8, 8, 8); camera.lookAt(0, 0, 0); camera.updateMatrixWorld();
  const scene = new THREE.Scene();
  const geometry = new THREE.BoxGeometry(1, 1, 1);
  const material = new THREE.MeshBasicMaterial();
  const mesh = new THREE.Mesh(geometry, material);
  mesh.position.set(origin.x, origin.y, origin.z); mesh.userData.deviceId = 'one'; scene.add(mesh);
  const store = new DeviceStore();
  store.add({ id: 'one', catalogId: 'one', kind: 'rack', surface, position: origin,
    rotation: { x: 0, y: 0, z: 0 }, ports: [], metadata: { label: 'Rack', powerWatts: null, heatBtuPerHour: null, rackUnits: null } });
  const dispose = useDeviceDrag({ renderer: { domElement: element }, camera, scene, pickTargets: [mesh],
    room: { width: 8, height: 3, depth: 6 } } as unknown as CanvasManager, store);
  cleanup = () => { dispose(); geometry.dispose(); material.dispose(); };
  function pointer(name: string, point: XYZ, pointerId = 1) {
    const screen = new THREE.Vector3(point.x, point.y, point.z).project(camera);
    listeners.get(name)?.({ clientX: 30 + (screen.x + 1) * 400, clientY: 20 + (1 - screen.y) * 300,
      pointerId, isPrimary: true, button: 0, preventDefault: vi.fn() });
  }
  return { store, pointer, element, captures, listeners, windowListeners, dispose };
}

describe('device dragging with real Three.js ray intersections', () => {
  it.each<MountSurface>(['floor', 'ceiling', 'north', 'south', 'east', 'west'])('snaps movement on %s and rolls back with Escape', surface => {
    const origin = { x: surface === 'east' ? 4 : surface === 'west' ? -4 : 0,
    ...
    const target = { x: origin.x + (surface === 'east' || surface === 'west' ? 0 : 0.74),
    ...
    expect(position.x).toBeCloseTo(origin.x + (target.x === origin.x ? 0 : 0.5));
    expect(position.y).toBeCloseTo(origin.y + (target.y === origin.y ? 0 : 0.5));
    expect(position.z).toBeCloseTo(origin.z + (target.z === origin.z ? 0 : 0.5));
    test.windowListeners.get('keydown')?.({ key: 'Escape', preventDefault: vi.fn() });
  ...
  it('does not snap an off-grid asset on a plain click', () => {
    ...
    test.pointer('pointerdown', origin); test.pointer('pointerup', origin);
    expect(test.store.get('one')!.position).toEqual(origin);
  });
